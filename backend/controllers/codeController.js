...
export const submitCode = async (req, res) => {
    ...
    for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
        try {
            ...
            // --- START: ROBUST LEADERBOARD UPDATE LOGIC ---
            const userIndex = contest.leaderboard.findIndex(p => p.userId.equals(req.user._id));

            if (userIndex !== -1) {
                // Work on a plain JS object copy to ensure changes are detected
                const userEntry = contest.leaderboard[userIndex].toObject();
                const oldTotalScore = userEntry.score;

                const problemScoreIndex = userEntry.problemScores.findIndex(ps => ps.problemId.toString() === problemId.toString());

                if (problemScoreIndex > -1) {
                    // Update existing problem score only if it's an improvement
                    if (currentProblemScore > userEntry.problemScores[problemScoreIndex].score) {
                        userEntry.problemScores[problemScoreIndex].score = currentProblemScore;
                    }
                } else {
                    // Add new problem score
                    userEntry.problemScores.push({ problemId, score: currentProblemScore });
                }

                // Always recalculate the total score from the problem scores array
                const newTotalScore = userEntry.problemScores.reduce((total, ps) => total + (ps.score || 0), 0);
                userEntry.score = newTotalScore;

                // Update timestamp only if the total score improved
                if (newTotalScore > oldTotalScore) {
                    userEntry.lastSuccessfulSubmissionTime = new Date();
                }

                // **THE FIX**: Replace the old object with the updated one in the array
                contest.leaderboard[userIndex] = userEntry;
                
                // Mark the array as modified and save
                contest.markModified('leaderboard');
                await contest.save();
                
                // Sort the leaderboard for emission
                const sortedLeaderboard = contest.leaderboard.sort((a, b) => {
                    if (a.score !== b.score) return b.score - a.score;
                    if (!a.lastSuccessfulSubmissionTime) return 1;
                    if (!b.lastSuccessfulSubmissionTime) return -1;
            ...
            // --- END: ROBUST LEADERBOARD UPDATE LOGIC ---
