...
const ContestRoom = () => {
    ...
    // Resizable panels
    const [leftPanelWidth, setLeftPanelWidth] = useState(50); // Start with a 50/50 split
    ...
    useEffect(() => {
        ...
        if (currentTestCase) {
            ...
            // Fallback to first test case if index is invalid
        ...
        try {
            ...
            if (type === 'run') {
                if (data.type === 'error') {
                    ...
                    // Compare result for 'run'
                    const expectedOutput = currentProblem.sampleTestCases?.[activeTestCaseIndex]?.output;
                    let actualOutput = data.output;
                    let parsedActual;
                    try {
                         // The sandbox returns a JSON string, so we need to parse it
                         parsedActual = JSON.parse(actualOutput);
                    } catch {
                         parsedActual = actualOutput; // If not JSON, use as is
                    }

                    // A simple string comparison after stringifying both
                    const isCorrect = JSON.stringify(parsedActual) === JSON.stringify(JSON.parse(expectedOutput));
                    setRunResult({
                        status: isCorrect ? 'accepted' : 'wrong',
                        input: editableInput,
                        expected: expectedOutput,
                        actual: JSON.stringify(parsedActual, null, 2),
                    });
                }
            } else { // Submit
                if(data.error){
    ...
    useEffect(() => {
        const onKeyDown = (e) => {
            if (e.ctrlKey || e.metaKey) {
                const activeEl = document.activeElement;
                const isFocusInEditor = editorWrapperRef.current && editorWrapperRef.current.contains(activeEl);
                const isFocusInConsoleInput = activeEl.tagName === 'TEXTAREA' && activeEl.closest('.console-input-area');

                if (e.key === "'") {
                    e.preventDefault();
                    if (!isRunning) handleRunSubmit('run');
                } else if (e.key === "Enter" && (isFocusInEditor || isFocusInConsoleInput)) {
                     e.preventDefault();
                    if (!isSubmitting) handleRunSubmit('submit');
                }
            }
    ...
    }, [isRunning, isSubmitting, editableInput, problemSolutions, questions, currentQuestionIndex, language, contest]);
            ...
            <main className="flex-1 flex flex-row overflow-hidden">
                ...
                <div style={{ width: `${leftPanelWidth}%` }} className="bg-gray-800 flex flex-col min-w-[300px]">
                     <div className="p-5 flex-1 overflow-auto">
                        ...
                        <div className="prose prose-invert prose-sm max-w-none text-gray-300" dangerouslySetInnerHTML={{ __html: currentProblem.description?.replace(/\n/g, '<br/>') }} />
                                ...
                                {consoleTab === 'result' && runResult && (
                                    <div className="font-mono text-sm">
                                        ...
                                        <div className="mt-4 space-y-3">
                                            {runResult.input && <div><div className="text-gray-400 text-xs uppercase font-semibold">Input</div><pre className="bg-gray-800 p-2 rounded mt-1">{runResult.input}</pre></div>}
                                            {runResult.expected && <div><div className="text-gray-400 text-xs uppercase font-semibold">Expected</div><pre className="bg-gray-800 p-2 rounded mt-1">{runResult.expected}</pre></div>}
                                            {runResult.actual && <div><div className="text-gray-400 text-xs uppercase font-semibold">Output</div><pre className="bg-gray-800 p-2 rounded mt-1">{runResult.actual}</pre></div>}
